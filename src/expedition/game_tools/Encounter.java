...
public class Encounter {
        ...
        while (menu) {
            ...
            if (in.matches("^(q|quit|exit)(\\s*)$")) {
                ...
                if ((input < 1) || (input > 4) || (currentChar.getAbilities()[input - 1] == null)) {
                    ...
                    Ability abil = currentChar.getAbilities()[input - 1];
                    //pick target now
                    boolean menu2 = true;
                    while (menu2) {
                        tprintln("Which ability would you like to use? (q/quit/exit to return)");
    ...
    private boolean run() throws InterruptedException {
        ...
        tprintln(currentChar.getName() + "'s speed is " + currentChar.getStatSpeed() + ", and the enemy's average is " + otherTeamSpeed + ". Are you sure you want to try to run?");
        if (inputAffirm(scan)) { // yes run
            if (currentChar.getStatSpeed() > otherTeamSpeed) { //If current character's speed is higher than the enemy's average, run away is successful.
                successfulRun();
            } else {
                HashMap<Boolean, Integer> a = new HashMap<>(); //If current character's speed is less, chance is lower (odds are char's speed to enemy's average)
    ...
    private boolean item() throws InterruptedException {
        if (currentChar.hasUsableItem()) {
            tprintln(currentChar.getName() + " is holding "
                    + "[" + currentChar.getEquippedConsumable() + "]"
                    + " : " + currentChar.getEquippedConsumable().getDescription());
            tprintln("Would you like to use it?");
            if (inputAffirm(scan)) {
                return currentChar.useEquippedItem();
            }
        } else {
    ...
    public void printFight(Character current) {
        ...
        for (Character p : player.list()) {
            tprintln("#" + (player.list().indexOf(p) + 1) + ": " + p.getName() + " - Level " + p.getLevel() + " " + p.getType() + "     Turns Until Play: (" + comboOrder.indexOf(p) + ")");
        ...
        for (Character p : other.list()) {
            tprintln("#" + (other.list().indexOf(p) + 1 + player.list().size()) + ": " + p.getName() + " - Level " + p.getLevel() + " " + p.getType() + "     Turns Until Play: (" + comboOrder.indexOf(p) + ")");
            tprintln("    Health: " + p.getHealthCurr() + "/" + p.getHealthMax() + " | Energy: " + p.getEnergyCurr() + "/" + p.getEnergyMax() + "");
            tprintln("");
        }
    }
